...
function opc_punto_atenc($id=''){
	return opc_sql("SELECT `idcatadeta`,concat(idcatadeta,' - ',descripcion) FROM `catadeta` WHERE idcatalogo=274 and estado='A'  valor IN(0,select subred from usuarios where id_usuario='{$_SESSION['us_sds']}') ORDER BY LENGTH(idcatadeta), idcatadeta",$id);
}
