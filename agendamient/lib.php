...
function cmp_agendamiento(){
 ...
 $c[]=new cmp('idp','n',18,$d['idpersona'],$w.' '.$key.' '.$o,'N° Identificación',0,'rgxdfnum','#################',true,$u,'','col-4');
...
function opc_punto_atenc($id=''){
	return opc_sql("SELECT `idcatadeta`,concat(idcatadeta,' - ',descripcion) FROM `catadeta` WHERE idcatalogo=274 and estado='A' and subred=(select subred from usuarios where id_usuario='{$_SESSION['us_sds']}') ORDER BY LENGTH(idcatadeta), idcatadeta",$id);
}
