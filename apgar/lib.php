...
function lis_tamApgar(){
	$info=datos_mysql("SELECT COUNT(*) total from hog_tam_apgar O LEFT JOIN personas P ON O.idpersona = P.idpersona where 1 ".whe_tamApgar() ." AND O.usu_creo ='".$_SESSION['us_sds']."'");
	$total=$info['responseResult'][0]['total'];
	$regxPag=12;
	$pag=(isset($_POST['pag-tamApgar']))? ($_POST['pag-tamApgar']-1)* $regxPag:0;
	$sql="SELECT ROW_NUMBER() OVER (ORDER BY 1) R,concat(O.idpersona,'_',O.tipodoc,'_',momento) ACCIONES,O.idpersona Documento,FN_CATALOGODESC(1,O.tipodoc) 'Tipo de Documento',CONCAT_ws(' ',P.nombre1,P.nombre2,P.apellido1,P.apellido2) Nombres,`puntaje` Puntaje,`descripcion` Descripcion 
...
function cmp_tamApgar(){
	...
	$c[]=new cmp('idpersona','n','20',$d['idpersona'],$w.' '.$o.' '.$key,'N° Identificación','idpersona',null,'',false,$u,'','col-2');
	$c[]=new cmp('tipodoc','s','3',$d['tipodoc'],$w.' '.$o.' '.$key,'Tipo Identificación','tipodoc',null,'',false,$u,'','col-25','getDatForm(\'apg\',\'person\',[\'datos\']);setTimeout(hiddxedad,300,\'edad\',\'cuestionario1\',\'cuestionario2\');');
