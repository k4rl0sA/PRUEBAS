...
function opc_asignado($id=''){
	...
	return opc_sql("SELECT `id_usuario`,concat(perfil,' - ',nombre) FROM `usuarios` WHERE `perfil`IN ('TSOREL','ENFREL','AUXREL','TOPREL','LEFREL','LARREL','RELENF') AND componente='EAC' AND subred IN(".$subred.") ORDER BY (2)",$id);
}
