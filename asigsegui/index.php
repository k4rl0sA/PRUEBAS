...
<title>Asignación Seguimiento || GIF-SDS</title><!--cambiar titulo por orden de fernando-->
...
$mod='asigsegui';//modulo
$rta=datos_mysql("select FN_USUARIO('".$_SESSION['us_sds']."') as usu;");
$usu=divide($rta["responseResult"][0]['usu']);
// var_dump($usu[1]);
$subred = ($usu[1]=='ADM') ? '1,2,3,4,5' : $usu[2] ;
$colaboradores=opc_sql("SELECT `id_usuario`,nombre FROM `usuarios` WHERE`perfil` IN ('ENF','AUX') AND componente='EAC' AND subred IN(".$subred.") ORDER BY 1",$_SESSION['us_sds']);
$estados=opc_sql("select idcatadeta,descripcion from catadeta where idcatalogo=38 and estado='A' order by 1",'');
?>
...
<div class='col-8 panel' id='<?php echo $mod; ?>'>
      <div class='titulo' >    ASIGNACION CASOS PARA SEGUIMIENTO <!-- CAMBIAR TITULO DE LA VENTANA -->
		<nav class='menu left' >
