...
function whe_homes() {
	...
	if (!empty($_POST['fpred'])) {
		...
		if ($_POST['fterri']) {
			...
			$sql .= " AND (H.territorio IN (SELECT A.territorio FROM adscrip where A.doc_asignado='{$_SESSION['us_sds']}') OR H.usu_creo = '{$_SESSION['us_sds']}' OR M.asignado_eac='{$_SESSION['us_sds']}' )";//OR D.doc_asignado='{$_SESSION['us_sds']}'
		}
		if ($_POST['fdigita']) {
			$sql .= " AND H.usu_creo ='" . $_POST['fdigita'] . "'";
		}
