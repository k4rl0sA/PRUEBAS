...
$o='prurap';
 ...
 $o='plancuidado';
   ...
   function lis_atencion(){
	...
	$id=divide($_POST['id']);
	$info=datos_mysql("SELECT COUNT(*) total FROM adm_facturacion F WHERE WHERE F.documento ='{$id[0]}' AND F.tipo_doc='{$id[1]}'");
	$total=$info['responseResult'][0]['total'];
	$regxPag=5;
...
// print_r($c);
	...
	if ($a=='datos-lis' && $b=='acciones'){
		...
		$rta.=$perfil;
		if (perfil1()=='MEDATE' || perfil1()=='ADM' || perfil1()=='ENFATE'|| perfil1()=='ADMEAC' || perfil1()=='SUPEAC' ){
