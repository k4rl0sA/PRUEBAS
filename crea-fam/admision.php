...
function focus_admision(){
  ...
  function lis_admision(){
	// var_dump($_POST['id']);
		// echo $sql;
	$id=divide($_POST['id']);	
	$sql="SELECT id_factura ACCIONES,`cod_admin` 'Codigo Ingreso', FN_CATALOGODESC(126,cod_cups) 'Codigo CUPS', FN_CATALOGODESC(127,final_consul) 'Finalidad de la Consulta' 
	FROM `adm_facturacion` WHERE tipo_doc ='{$id[1]}' and documento ='{$id[0]}'";
	// echo $sql;
