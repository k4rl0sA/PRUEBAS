...
function gra_person(){
	...
	function opc_numfam($id=''){
	 ...
	 function opc_tipo_doc($id=''){
	    return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=1 and estado='A' ORDER BY CAST(idcatadeta AS UNSIGNED)",$id);
    }
...
// var_dump($a);
	if ($a=='homes' && $b=='acciones'){
		...
		$rta.="<li title='Crear Ubicación de la Familia'  Onclick=\"mostrar('homes','pro',event,'','lib.php',7,'homes');setTimeout(DisableUpdate,300,'fechaupd','hid');Color('homes-lis');\"><i class='fa-solid fa-circle-plus ico' id='".$c['ACCIONES']."'></i></li>";		
	}
 	if ($a=='famili-lis' && $b=='acciones'){
		...
		$rta.="<li title='Mostrar Integrantes' Onclick=\"mostrar('person1','fix',event,'','lib.php',0,'person1');Color('famili-lis');setTimeout(crearMenu,1000);\"><i class='fa-solid fa-people-group ico' id='".$c['Cod_Familiar']."'></i></li>";
		
		$rta.="<li title='Planes de Cuidado Familiar' Onclick=\"mostrar('planDcui','pro',event,'','plancui.php',7);Color('famili-lis');\"><i class='fa-solid fa-file-contract ico'  id='".$c['Cod_Familiar']."'></i></li>";
	...
	if ($a=='datos-lis' && $b=='acciones'){
		$rta="<nav class='menu right'>";
		$rta.="<li class='icono menubtn' id='menuToggle_".$c['ACCIONES']."'></li><div id='menuContainer'></div>";
		...
		$rta.="<li class='icono aten1' title='Crear Atención' id='".$c['ACCIONES']."' Onclick=\"mostrar('atencion','pro',event,'','atencion.php',7,'atencion');\"></li>";
		}
