...
function cmp_segnoreg(){
  ...
  $c[]=new cmp('fecha_obs','d',10,$d,$w.' '.$bl.' '.$ge.' nO '.$o,'Fecha de Evento Obstetrico','fecha_obs',null,null,true,true,'','col-2');
  ...
  $c[]=new cmp('edad_gest','s',3,$d,$w.' '.$bl.' '.$ge.' '.$o,'Edad Gestacional Al Momento De Identificacion En Semanas','edad_gest',null,null,true,true,'','col-4','enabEmbPue();');
  $c[]=new cmp('resul_gest','s',3,$d,$w.' '.$bl.' Pue '.$no.' '.$o,'Resultado De La Gestación','resul_gest',null,null,true,false,'','col-2',"enabNV();");
  $c[]=new cmp('peso_nacer','sd',5,$d,$w.' '.$bl.' Nav '.$no.' '.$o,'Peso Al Nacer (Gr)','peso_nacer',null,null,false,false,'','col-2');
  $c[]=new cmp('asist_controles','s',3,$d,$w.' '.$bl.' '.$no.' '.$o,'¿Asiste A Controles Prenatales?','rta',null,null,true,true,'','col-2');
  $c[]=new cmp('exa_labo','s',3,$d,$w.' '.$bl.' '.$no.' '.$o,'¿Cuenta Con Exámenes De Laboratorio Al Día? Con Relación Al Trimestre Gestacional?','rta',null,null,true,true,'','col-4');
  $c[]=new cmp('cons_micronutri','s',3,$d,$w.' '.$bl.' '.$no.' '.$o,'¿Consume Micronutrientes?','rta',null,null,true,true,'','col-4');
  $c[]=new cmp('esq_vacu','s',3,$d,$w.' '.$bl.' '.$no.' '.$o,'¿Tiene Esquema De Vacunacion Completo Para La Eg?','rta',null,null,true,true,'','col-3');
  ...
  $men = ($p['mes']<216) ? false : true ;
  $c[]=new cmp('sistolica','sd',3,$d,$w.' '.$q,'Valor Sistolica (50 mmhg y 200 mmhg)','sistolica','rgxsisto',null,true,$men,'','col-25');
  $c[]=new cmp('diastolica','sd',3,$d,$w.' '.$q,'Valor Diastolica (30 mmhg y 150 mmhg)','diastolica','rgxdiast',null,true,$men,'','col-25');
...
function opc_clasi_nutri($id=''){
 ...
 function opc_ser_remigesti($id=''){
	...
	function formato_dato($a,$b,$c,$d){
      ...
      $rta.="<li class='icono editar' title='Editar' id='".$c['ACCIONES']."' Onclick=\"setTimeout(getData,500,'segnoreg',event,this,['fecha_seg','segui','estado_seg','prioridad','aten_med','aten_par','sistolica','diastolica','frec_cardiaca','frec_respiratoria','saturacion','gluco','peso','talla','clasi_nut','ser_remigesti'],'../etnias/embsegnoreg.php');setTimeout(enabPrioEtn,700);enbValue('id','segnoreg','".$c['ACCIONES']."');enaFie(document.getElementById('observaciones'),false);\"></li>";
			}
		return $rta;
	}
