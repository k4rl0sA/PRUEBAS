...
// $rta='';
  ...
  // var_dump($_POST);
    ...
    $c[]=new cmp('segui','s',3,$d,$w.' '.$o,'Seguimiento N°','segui',null,null,true,true,'','col-2',"staEfe('segui','sta');");
    $c[]=new cmp('estado_seg','s',3,$d,$w.' sTa '.$o,'Estado','estado_seg',null,null,true,true,'','col-2',"EnabEfec(this,['datiden','infoserv','detsegh','detsegp','aspfin'],['Ob'],['nO'],['bL']);,enabFielSele(this,true,['motivo_estado'],['3']);");
...
function get_seguim(){
  if($_REQUEST['id']==''){
    ...
    // print_r($id);
    $sql="SELECT idseg, fecha_seg, segui, estado_seg, motivo, interven, gestante, edad_gest, Nom_fami, tipo_doc, num_doc, paren, tel_conta, ubi, ser_req, fecha_ing, uss_ing, motivo_cons, uss_tras, ing_unidad, ante_salud, imp_diag, uss_encu, servicio_encu, imp_diag2, nece_apoy, espe1, espe2, adh_tto, observaciones
          FROM `emb_segui` 
          WHERE idseg='{$id[0]}'";
