...
function cmp_seguim(){
   $rta="<div class='encabezado seguiEmbera'>TABLA SEGUIMIENTOS</div><div class='contenido' id='seguim-lis'>".lis_seguim()."</div>";
// $rta='';
  $w="seguim";
	$o='modini';
  $bl='bL';
...
function get_seguim(){
  if($_REQUEST['id']==''){
    ...
    // print_r($id);
    $sql="SELECT null,idseg,P.sexo,fecha_seg, segui, estado_seg, motivo, interven, gestante, edad_gest, Nom_fami, S.tipo_doc, num_doc, paren, tel_conta, ubi, ser_req, fecha_ing, uss_ing, motivo_cons, uss_tras, ing_unidad, ante_salud, imp_diag, uss_encu, servicio_encu, imp_diag2, nece_apoy, espe1, espe2, adh_tto, observaciones
          FROM `emb_segui` S
