...
// $rta='';
  ...
  $c[]=new cmp('sexo','h',1,$p['sexo'],' GeST ','sexo','sexo',null,'',false,false,'','col-1');
    ...
    $c[]=new cmp('edad_gest','s',3,$d,$w.' EGe '.$bl.' '.$o,'Edad Gestacional (Semanas)','edad_gesta',null,null,false,false,'','col-2');
    ...
    $c[]=new cmp('fecha_ing','d',10,$d,$w.' '.$bl.' '.$o,'Fecha De Ingreso','fecha_ing',null,null,false,true,'','col-2',"validDate(this,$days,0);");
...
function get_seguim(){
  if($_REQUEST['id']==''){
    ...
    // print_r($id);
    $sql="SELECT P.sexo,idseg, fecha_seg, segui, estado_seg, motivo, interven, gestante, edad_gest, Nom_fami, S.tipo_doc, num_doc, paren, tel_conta, ubi, ser_req, fecha_ing, uss_ing, motivo_cons, uss_tras, ing_unidad, ante_salud, imp_diag, uss_encu, servicio_encu, imp_diag2, nece_apoy, espe1, espe2, adh_tto, observaciones
          FROM `emb_segui` S
...
function formato_dato($a,$b,$c,$d){
  ...
  if ($a=='seguim' && $b=='acciones'){
    ...
    $rta.="<li class='icono editar' title='Editar' id='".$c['ACCIONES']."' Onclick=\"setTimeout(getData,500,'seguim',event,this,['fecha_seg'],'../etnias/embsegui.php');enbValue('idseg','seguim','".$c['ACCIONES']."');enaFie(document.getElementById('observaciones'),false);\"></li>";
    }
		return $rta;
}
