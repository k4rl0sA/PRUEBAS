...
function cmp_geoloc(){
	...
	$c[]=new cmp('zona','s',3,$d['localidad'],$w.' '.$o,'Localidad','localidad',null,'',false,false,'','col-25');
...
function get_predio(){
	// print_r($_POST);
	$id=divide($_POST['id']);
	$sql="SELECT idgeo,G.zona, G.localidad, G.upz, G.barrio, G.sector_catastral, G.nummanzana, G.predio_num, G.unidad_habit, G.direccion, G.vereda, G.cordx, G.cordy, G.territorio 
 	FROM `hog_geo` G 
  	WHERE G.idgeo ='".$id[0]."'";
