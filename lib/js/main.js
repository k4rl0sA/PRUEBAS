...
function mostrar(tb, a='', ev, m='', lib=ruta_app, w=7, tit='', k='0') {
	var id = tb+'-'+a;
	if (a == 'pro') {
        if (ev!=undefined) {k=ev.target.id;}else{tit=document.querySelector('.content.content-2 .title.txt-center h2').innerText;}
...
function myFetch(b, c, d) {
  ...
  function typeErrors(rta){
	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		let timeout = 500; // Tiempo por defecto para mensajes normales
		try {
			if (data.includes('&a=men&')) {
				Toast(true, 'fas fa-redo fa-spin check', 'Generando', 'Ventana con los elementos.', 500); // Mostrar mensaje de "Generando" durante 1.5 segundos
				timeout = 500; // Ajustar el tiempo para este mensaje
			} else {
				Toast(true, 'fas fa-redo fa-spin check', 'Cargando', 'Generando Elementos.', 250); // Mostrar mensaje de "Cargando" durante 500 ms
				timeout = 250; // Tiempo de 0.5 segundos para mensajes normales
			}
			...
			const response = await fetch(path, {
				...
				body: data, // No JSON.stringify para application/x-www-form-urlencoded
			});
			...
			if (response.status === 500) {
				Toast(true, 'fas fa-exclamation-circle danger', 'Error :', 'Internal Server Error', 5000, false); // Mostrar error 500 durante 5 segundos
				timeout = 5000; // Ajustar el tiempo para errores a 5 segundos
				return;
			}
			...
			const responseData = await response.text(); // Usar .text() en lugar de .json() para respuestas en texto plano
			...
			console.error("Error:", error);
			Toast(true, 'fas fa-exclamation-circle danger', 'Error', error.message, 5000, false); // Mostrar errores por 5 segundos
			timeout = 5000; // Ajustar el tiempo para errores
		} finally {
			// Ocultar el Toast después del tiempo apropiado
			setTimeout(() => {
				Toast(false); // Ocultar el Toast después de que se haya mostrado el mensaje
			}, timeout); // Usar el tiempo dinámico basado en el tipo de mensaje
		}
	}
