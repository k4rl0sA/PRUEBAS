	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		...
		try {
			if (data.includes('&a=men&')) {
				Toast(true, 'fas fa-redo fa-spin check', 'Generando', 'Ventana con los elementos.', 1000); // Mostrar mensaje de "Generando" durante 1.5 segundos
				timeout = 1500; // Ajustar el tiempo para este mensaje
			} else {
				Toast(true, 'fas fa-redo fa-spin check', 'Cargando', 'Generando tabla de elementos.', 250); // Mostrar mensaje de "Cargando" durante 500 ms
