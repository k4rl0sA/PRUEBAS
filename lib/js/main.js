...
function startImport(file,ncol,tab,imp) {
	...
	.then(body => {
		...
		function processText({ done, value }) {
			...
			buffer = parts.pop(); // Retiene cualquier fragmento parcial
			const errorsAll = []; 
			...
			reader.read().then(processText);  // Continuar leyendo el siguiente chunk
		}
...
function updateMessageGradually(element, newMessage, delay = 50) {
    let currentMessage = '';
    let index = 0;
    function update() {
        if (index < newMessage.length) {
            currentMessage += newMessage[index];
            element.textContent = currentMessage;
            index++;
            setTimeout(update, delay);  // Controla la velocidad de actualización con el retraso
        }
    }
    update();  // Inicia la actualización gradual
}
...
function handleServerResponse(json, errorsAll, index, endInd) {
    if (json.status === 'progress') {
        ...
        if (json.errors) {
            errorsAll.push(json.errors);
            updateMessageGradually(statusMessage, json.errors.join(' ')); // Muestra los errores gradualmente
        }
    } else if (json.status === 'success') {
        updateProgress(json.progress);
        updateMessageGradually(statusMessage, json.message);  // Muestra el mensaje de éxito gradualmente
        if (json.errors) {
            errorsAll.push(json.errors);
        }
    ...
    if (index === endInd) {
        updateMessageGradually(statusMessage, "Errores:\n" + errorsAll.join("\n")); // Muestra todos los errores acumulados gradualmente
    }
}

function updateProgress(newProgress) {
    let currentProgress = parseInt(progressBar.style.width) || 0;  // El progreso actual en la barra
    function updateGradually() {
        if (currentProgress < newProgress) {
            currentProgress += 1;  // Incrementa el progreso paso a paso
            progressBar.style.width = `${currentProgress}%`;  // Actualiza el ancho de la barra
            progressText.textContent = `${currentProgress}% completado`;  // Actualiza el texto
            setTimeout(updateGradually, 50);  // Ajusta el tiempo de retraso según sea necesario
        }
    }
    updateGradually();
}
