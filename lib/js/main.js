	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		try {
			if (data.includes('&a=men&')) {
				Toast(true, 'fas fa-redo fa-spin check', 'Generando', 'Ventana con los elementos.', 1500);
			} else {
				Toast(true, 'fas fa-redo fa-spin check', 'Cargando', 'Generando tabla de elementos.', 1500);
			}
			...
			if (response.status === 500) {
				Toast(true, 'fas fa-exclamation-circle danger', 'Error :', 'Internal Server Error', 3000, false);
				return; // Evitar seguir con el flujo después de mostrar el error
			}
			...
			console.error("Error:", error);
			// Mostrar el Toast con un tiempo más largo (ejemplo: 3000 ms) en caso de error
			Toast(true, 'fas fa-exclamation-circle danger', 'Error', error.message, 3000, false);
		} finally {
			// Asegurarse de que se oculta el Toast después del tiempo especificado.
			setTimeout(() => {
				Toast(false);
			}, 3500); // Un tiempo ligeramente mayor para asegurar que se complete el Toast anterior.
		}
	}
