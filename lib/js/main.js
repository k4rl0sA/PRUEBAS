...
function myFetch(b, c, d) {
  ...
  function typeErrors(rta){
	...
	/* function setAutoHideToast(toast, progress, delay = 3000) {
    	let timer1, timer2;
	...
	function Toast(toast, progress,flag) {
		const overl = document.getElementById('overlay');
		overl.classList.toggle('visible');
		overl.style.visibility = overl.style.visibility === 'visible' ? 'hidden' : 'visible';
		if(flag){
			if (toast && progress) {
				const toastImg = document.querySelector('.toast-content');
				const toastTxt1 = document.querySelector('.text-1');
				const toastTxt2 = document.querySelector('.text-2');
				toastImg.textContent ="<i class='fas fa-solid fa-check check'></i>";
				toastTxt1.textContent = 'Cargando';
				toastTxt2.textContent = 'Generando tabla de elementos.';
				toastImg.appendChild(icon);
	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		...
		try {
			...
			const loader = document.getElementById("loader");
			Toast(toast, progress,false);
			//setCloseIconListener(closeIcon, toast, progress);
		}
	}
