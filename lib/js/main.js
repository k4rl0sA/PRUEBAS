...
function myFetch(b, c, d) {
  ...
  function typeErrors(rta){
	...
	function Toast(show,icon,tit,msj,progress=true) {
		if (show) {
			...
			if (toast) {
				setTimeout(() => {toast.classList.remove("active");}, 500);
	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		try {
			Toast(true,'fas fa-redo fa-spin check','Cargando','Generando tabla de elementos.');
