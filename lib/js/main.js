	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		...
		try {
			if (data.includes('&a=men&')) {
				Toast(true, 'fas fa-redo fa-spin check', 'Generando', 'Ventana con los elementos.', 1500); // Mostrar mensaje de "Generando" durante 1.5 segundos
				timeout = 1500; // Ajustar el tiempo para este mensaje
			} else {
				Toast(true, 'fas fa-redo fa-spin check', 'Cargando', 'Generando tabla de elementos.', 500); // Mostrar mensaje de "Cargando" durante 500 ms
				timeout = 500; // Tiempo de 0.5 segundos para mensajes normales
			}
