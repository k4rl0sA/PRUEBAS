...
function handleServerResponse(json, errorsAll, index, endInd) {
	if (json.status === 'progress') {
		updateProgress(json.progress,json);
	...
	if (index === endInd) {
		// statusMessage.innerHTML = `Se han insertado ${json.successful} registros correctamente de ${json.total} en Total<br>Errores:<br>${errorsAll.join("<br>")}`;
...
function updateProgress(json) {
    ...
    function updateGradually() {
        if (currentProgress < json.progress) {
