...
function myFetch(b, c, d) {
  ...
  function typeErrors(rta){
	...
	function setCloseToast(closeIcon, toast, progress, timers) {
		if (closeIcon) {
	...
	function showToast(toast, progress) {
		if (toast && progress) {
	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		try {
			...
			showToast(toast, progress);
			timers = setAutoHideToast(toast, progress);
			...
			const loader = document.getElementById("loader");
			setCloseToast(closeIcon, toast, progress, timers);
		}
	}
