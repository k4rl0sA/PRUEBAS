	...
	async function pFetch(path, data, callback, method = "POST", headers = {}) {
		try {
			...
			if (response.status === 500) {
				Toast(true, 'fas fa-exclamation-circle danger', 'Error :', 'Internal Server Error', 2000, false);
				return;
			}
			...
			console.error("Error:", error);
			Toast(true, 'fas fa-exclamation-circle danger', 'Error', error.message, 3000, false); // Mostrar el error
		} finally {
			setTimeout(() => {
				Toast(false);
			}, 1000);
