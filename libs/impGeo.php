	...
	if (strtolower(end($ext)) !== "csv") {
		...
		if($_POST['tab']!=='geografico'){
			...
			if ($handle === FALSE) {
				...
				/* $rta = array(
					'type' => 'Error','msj'=>'Registro #' . ($nFil - 1) . ' '.$sql 
				);
				response ($rta); */
						$r = dato_mysql($sql);				
						if (preg_match('/Error/i', $r)){
							$rta = array(
								'type' => 'Error','msj'=>'Registro #' . ($nFil - 1) . ' '.$r
							);
							response ($rta);
						}
					}
...
function asignados(){
	$info=datos_mysql("SELECT id_usuario FROM usuarios WHERE componente IN('HOG') and subred in (SELECT subred FROM usuarios where id_usuario='".$_SESSION['us_sds']."' and estado='A') ");
	$valores = array(); 
	if (is_array($info['responseResult'])) {
		foreach ($info['responseResult'] as $item) {
