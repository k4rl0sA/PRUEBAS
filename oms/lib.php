...
function lis_tamoms(){
	$info=datos_mysql("SELECT COUNT(*) total from hog_tam_oms O LEFT JOIN personas P ON O.idpersona = P.idpersona where 1 ".whe_tamoms() ." AND O.usu_creo ='".$_SESSION['us_sds']."'");
	$total=$info['responseResult'][0]['total'];
	$regxPag=12;
	$pag=(isset($_POST['pag-tamoms']))? ($_POST['pag-tamoms']-1)* $regxPag:0;
	$sql="SELECT ROW_NUMBER() OVER (ORDER BY 1) R,concat(O.idpersona,'_',O.tipodoc) ACCIONES,O.idpersona Documento,FN_CATALOGODESC(1,O.tipodoc) 'Tipo de Documento',CONCAT_ws(' ',P.nombre1,P.nombre2,P.apellido1,P.apellido2) Nombres,`puntaje` Puntaje,descripcion  
	FROM hog_tam_oms O 
...
function cmp_tamoms(){
	...
	$c[]=new cmp('idpersona','n','20',$d['idpersona'],$w.' '.$o.' '.$key,'N° Identificación','idpersona',null,'',false,$u,'','col-3');
	$c[]=new cmp('tipodoc','s','3',$d['tipodoc'],$w.' '.$o.' '.$key,'Tipo Identificación','tipodoc',null,'',false,$u,'','col-3','getDatForm(\'oms\',\'person\',\'datos\');setTimeout(hiddxedad,500,\'edad\',\'pruoms\');');
