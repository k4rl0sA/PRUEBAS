...
function lis_psicologia(){
	...
	LEFT JOIN usuarios U ON S.doc_asignado = U.id_usuario
	LEFT JOIN eac_rutpsico R ON S.documento = R.documento AND S.tipo_doc = R.tipo_doc
	WHERE (A.atencion_ordenpsicologia='SI' OR R.asigno='SI' ) and S.doc_asignado IN ('{$_SESSION['us_sds']}')";//
	$sql.=whe_psicologia();
...
function cmp_psicologia() {
	...
	$c[]=new cmp('fecha_ses1','d','10',$d['fecha_ses1'],$w.' '.$o,'Fecha','fecha_ses1',null,'',true,$u,'','col-2','validDate(this,-19,0)');
...
function cmp_sesion2() {
	...
	$c[]=new cmp('psi_fecha_sesion','d','10',$d['psi_fecha_sesion'],$w.' '.$o,'Fecha de la Sesion','psi_fecha_sesion',null,null,true,$u,'','col-4','validDate(this,-19,0);');
	...
	$c[]=new cmp('contin_caso','s','3',$d['contin_caso'],$w.' '.$o,'Continuidad del caso','contin_caso',null,null,$u,$u,'','col-10');
	for ($i=0;$i<count($c);$i++) $rta.=$c[$i]->put();
	return $rta;
}
...
function cmp_sesion_fin() {
	...
	$c[]=new cmp('psi_fecha_sesion','d','10',$j['psi_fecha_sesion'],$w.' '.$o,'Fecha de la Sesion','psi_fecha_sesion',null,null,true,$ob,'','col-10','validDate(this,-19,0);');
