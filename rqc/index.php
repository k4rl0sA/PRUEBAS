...
function hiddxedad(srq_edad,cuestionario1,cuestionario2) {
	const edad=document.getElementById(srq_edad);
	const cmpHid1 = document.querySelectorAll(`.${cuestionario1}`);
	const cmpHid2 = document.querySelectorAll(`.${cuestionario2}`);

	if(edad.value > 0 && edad.value < 17){
		for(i=0;i<cmpHid1.length;i++){
			cmpHid1[i].classList.remove('oculto');
		}
		for(i=0;i<cmpHid2.length;i++){
			cmpHid2[i].classList.add('oculto');
		}
	}else{
		for(i=0;i<cmpHid1.length;i++){
			cmpHid1[i].classList.add('oculto');
		}
		for(i=0;i<cmpHid2.length;i++){
			cmpHid2[i].classList.remove('oculto');
		}
	}
...
<div class='col-8 panel' id='<?php echo $mod; ?>'>
      <div class='titulo' >TAMIZAJE SRQ - RQC
		<nav class='menu left' >
			<li class='icono actualizar'    title='Actualizar'      Onclick="actualizar();">
			<li class='icono filtros'    title='Filtros'      Onclick="showFil(mod);">
			<li class='icono crear'       title='Crear TAMIZAJE SRQ Y RQC'     Onclick="mostrar(mod,'pro');"></li> <!--setTimeout(load,500);-->
		</nav>
