...
function lis_rute(){
	...
	$sql="SELECT ROW_NUMBER() OVER (ORDER BY 1) R,`id_ruteo` ACCIONES,FN_CATALOGODESC(2,localidad) localidad,sector_catastral 'Sector Catastral',nummanzana Manzana,predio_num predio,nombres,`fecha_asig` Asignado,FN_CATALOGODESC(191,priorizacion) Priorización,estado_g Gestionado,estado
  FROM `eac_ruteo` 
  WHERE 1 ";
...
//  var_dump($d['estado_g']);
 ...
 $c[]=new cmp('fecha_consulta','t','90',$d['direccion'],$w.' '.$o,'Direccion','fecha_consulta',null,null,false,$u,'','col-4');
 $c[]=new cmp('cod_cups','n','10',$d['cod_cups'],$w.' '.$o,'Telefono 1','cod_cups',null,null,false,$u,'','col-2');
 $c[]=new cmp('per_consul','n','10',$d['per_consul'],$w.' '.$o,'Telefono 2','per_consul',null,null,false,$u,'','col-2');
 c[]=new cmp('telefono3','n','10',$d['telefono3'],$w.' '.$o,'Telefono 3','telefono3',null,null,false,$u,'','col-2');
...
//  $c[]=new cmp('gestion','s',2,$d['gestion'],$w.' '.$o,'Gestion','gestion',null,null,true,true,'','col-2',"enabOthSi('gestion','pRe')");
 ...
 $c[]=new cmp('estado_g','s',2,$d['estado_g'],$w.' pRe '.$o,'estado','estado_g',null,null,true,$x,'','col-2',"tipVivi('estado_g','StG');enabStatPreAdm(this,['motivo_estado']);");//
