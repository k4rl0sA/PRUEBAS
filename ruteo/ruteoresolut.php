...
function opc_usuariocod_admin(){
	...
	if($_REQUEST['id']!=''){
		$id=divide($_REQUEST['id']);
		$sql="SELECT f.cod_admin cod,concat(f.cod_admin,' - ',FN_CATALOGODESC(127,f.final_consul)) FROM adm_facturacion f WHERE f.tipo_doc='{$id[0]}' AND f.documento='{$id[1]}' ORDER BY 1";
		$info=datos_mysql($sql);
		// print_r($sql);
		return json_encode($info['responseResult']);
	} 					
...
function opc_cod_admin($id=''){
	...
	if ($id==''){
		...
		$co=divide($id);
		// var_dump($sql);
		return opc_sql("SELECT f.cod_admin cod,concat(cod_admin,' - ',FN_CATALOGODESC(127,final_consul)) FROM adm_facturacion f WHERE cod_admin='$id'", $id);
	}
