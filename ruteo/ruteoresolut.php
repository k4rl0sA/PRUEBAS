...
//  var_dump($d);
 ...
 $c[]=new cmp('estado','s',3,$d['predio'],$w.' PuE StG '.$o,'estado','estado',null,null,true,$u,'','col-2',"changeSelect('estado','famili');");
 $c[]=new cmp('famili','s',3,$d['famili'],$w.' PuE StG '.$o,'famili','famili',null,'',true, $u,'','col-3',"changeSelect('famili','usuario');");//N° FAMILIA
 $c[]=new cmp('usuario','s',3,$d['usuario'],$w.' PuE StG '.$o,'usuario','usuario',null,'',true,$u,'','col-3',"changeSelect('usuario','cod_admin');"); //TIPO_DOC,DOCUMENTO Y NOMBRE USUARIO
 $c[]=new cmp('cod_admin','s',3,$d['cod_admin'],$w.' PuE StG '.$o,'cod_admin','cod_admin',null,'',true,$u,'','col-2');//traer los codigos del usuario de atencion
...
function opc_usuario($id=''){
	...
	if ($id==''){
		...
		$co=divide($id);
		$sql="CONCAT_WS('_',tipo_doc,idpersona),CONCAT_WS('-',idpersona,tipo_doc,CONCAT_WS(' ',nombre1,apellido1)) FROM personas p WHERE tipo_doc='$co[0]' AND p.idpersona='$co[1]'";
		var_dump($sql);
		return opc_sql("CONCAT_WS('_',tipo_doc,idpersona),CONCAT_WS('-',idpersona,tipo_doc,CONCAT_WS(' ',nombre1,apellido1)) FROM personas p WHERE tipo_doc='$co[0]' AND p.idpersona='$co[1]'", $id);
	}
