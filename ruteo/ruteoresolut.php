...
function cmp_ruteresol(){
 ...
 $c[]=new cmp('estado','s',3,'',$w.' PuE StG '.$o,'estado','estado',null,null,false,true,'','col-2',"changeSelect('estado','famili');");
 ...
 $c[]=new cmp('cod_admin','s',3,$d['cod_admin'],$w.' PuE StG '.$o,'cod_admin','cod_admin',null,'',false, true,'','col-2');//traer los codigos del usuario de atencion
...
function opc_estado($id=''){
	$id=opc_idgeo($_REQUEST['id']);
		$co=divide($id);
		var_dump("SELECT idgeo,FN_CATALOGODESC(44,estado_v)
		from hog_geo where 
		sector_catastral='$co[0]' AND nummanzana='$co[1]' AND predio_num='$co[2]' AND unidad_habit='$co[3]' AND estado_v>3");
