...
function lis_rute(){
	$info=datos_mysql("SELECT COUNT(*) total from eac_ruteo er LEFT JOIN hog_geo G ON er.idgeo = G.idgeo LEFT JOIN apro_terr A ON G.idgeo = A.idgeo ".whe_rute());
...
/* function lis_rute(){
  ...
  function opc_perfil_gest($id=''){
	return opc_sql('SELECT idcatadeta,descripcion FROM catadeta WHERE idcatalogo=218 and estado="A" AND  perfil=(select perfil from id_usuario where id_usuario='{$_SESSION['us_sds']}') ORDER BY 1',$id);
	}
