...
var_dump($d);
 ...
 $c[]=new cmp('fecha_asig','d','10',$d['fecha_asig'],$w.' '.$o,'FECHA ASIGNACIÓN','fecha_asig',null,null,false,$u,'','col-15');
 ...
 $c[]=new cmp('documento','nu','999999999999999999',$d['documento'],$w.' '.$o,'NÚMERO DE DOCUMENTO','documento',null,null,false,$u,'','col-2');
 ...
 */
 $o='gesefc';
 $c[]=new cmp($o,'e',null,'PROCESO DE GESTIÓN',$w);
 $c[]=new cmp('estado_agenda','s',2,'',$w.' sTA '.$o,'Estado','estado_agenda',null,null,true,false,'','col-4','enabRutAgen();enabRutRech();');//
 $c[]=new cmp('motivo_estado','s','3','',$w.' ReC '.$o,'motivo_estado','motivo_estado',null,null,false,false,'','col-4','enabRutOthSub();');//
...
function lista_gestion(){ //revisar
	// var_dump($_POST);
	$id=divide($_POST['id']);
		$sql="SELECT id_rutges,erg.fecha_llamada 'Fecha',FN_CATALOGODESC(35,estado_llamada) 'Estado de la LLamada',FN_CATALOGODESC(35,estado_agenda) 'Estado de la Agenda',erg.doc_asignado 'Asignado A', fecha_create 'Creó'
...
function get_rute(){
	if($_POST['id']=='0'){
		...
		$id=divide($_POST['id']);
		$sql="`id_ruteo`,R.`idgeo`,`fuente`,`fecha_asig`,`priorizacion`,tipo_prior, `tipo_doc`, `documento`, `nombres`, `sexo`, R.direccion,`telefono1`, `telefono2`, `telefono3`, G.`subred`, G.`localidad`, G.`upz`, G.`barrio`, G.sector_catastral, G.nummanzana, G.predio_num, G.unidad_habit, G.`cordx`, G.`cordy` 
		FROM `eac_ruteo` R 
