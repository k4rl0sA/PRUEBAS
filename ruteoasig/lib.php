...
function focus_asigruteo(){
 return 'asigruteo';
}


function men_asigruteo(){
 $rta=cap_menus('asigruteo','pro');
 return $rta;
}
...
function cap_menus($a,$b='cap',$con='con') {
  ...
  $acc=rol($a);
  //print_r($acc);
  if ($a=='asigruteo'  && isset($acc['crear']) && $acc['crear']=='SI'){  
...
function cmp_asigruteo(){
 ...
 'cordx'=>'','cordy'=>'','estrato'=>'','asignado'=>'','estado_v'=>'','motivo_estado'=>'','tipo_doc'=>'','documento'=>'','nombres'=>'','telefono1'=>'','telefono2'=>'','telefono3'=>'','priorizacion'=>'','evento2'=>'','evento3'=>'','evento4'=>''];
 ...
 $o='infpri';
 $c[]=new cmp($o,'e',null,'GRUPO DE POBLACIÓN PRIORIZADA',$w);
 ...
 $c[]=new cmp('fuente','s',3,$d['fuente'],$w.' '.$o,'fuente','fuente',null,null,false,false,'','col-25');
 $c[]=new cmp('priorizacion','s',3,$d['priorizacion'],$w.' '.$o,'priorizacion','priorizacion',null,null,false,false,'','col-25');
 $o='infgen';
...
function opc_asignado($id=''){
	...
	return opc_sql("SELECT `id_usuario`,nombre FROM `usuarios` WHERE componente='EAC' AND `perfil` in('MEDATE','ENFATE','PSIEAC') AND subred=(SELECT subred FROM usuarios WHERE id_usuario='{$_SESSION['us_sds']}') ORDER BY 2",$id);
}
...
function get_asigruteo(){
	if($_POST['id']=='0'){
		...
		FROM `hog_geo` "; */
		$sql="SELECT tipo_doc, documento, nombres,telefono1, telefono2, telefono3,D.estrategia,D.subred,D.localidad,D.upz,D.barrio,D.territorio,D.microterritorio,D.sector_catastral,D.direccion,D.direccion_nueva,D.nummanzana,D.predio_num,D.unidad_habit,D.vereda,vereda_nueva,D.motivo_estado,D.estado_v,D.zona,D.asignado,D.cordx,D.cordy,D.estrato,fuente,priorizacion 
...
// var_dump($a);
// var_dump($rta);
	if ($a=='asigruteo' && $b=='acciones'){
		$rta="<nav class='menu right'>";		
		$rta.="<li class='icono mapa1' title='Editar Información Geografica' id='".$c['ACCIONES']."' Onclick=\"mostrar('asigruteo','pro',event,'','lib.php',7);\"></li>"; //setTimeout(hideExpres,1000,'estado_v',['7']);
	}
