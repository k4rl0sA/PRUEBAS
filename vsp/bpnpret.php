...
function focus_bpnpret(){
 ...
 function cap_menus($a,$b='cap',$con='con') {
  $rta = "";
  $acc=rol($a);
  if ($a=='bpnpret' && isset($acc['crear']) && $acc['crear']=='SI') {  
...
$ob='Ob';
  ...
  $c[]=new cmp('sexo','h','50',$p['sexo'],$w.' '.$o,'sexo','sexo',null,'',false,false,'','col-1');
    ...
    $c[]=new cmp('clasi_nutri','s','3',$d,$w.' '.$ob.' '.$o,'Clasificacion Nutricional','clasi_nutri',null,null,false,$x,'','col-3');
...
function gra_bpnpret(){
  ...
  function get_bpnpret(){
    if($_REQUEST['id']==''){
      ...
      $id=divide($_REQUEST['id']);
      $sql="SELECT concat_ws('_',id_bpnpret,D.idpeople,numsegui,evento),
