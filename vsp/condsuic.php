...
function focus_condsuic(){
 ...
 FUNCTION seg_condsuic(){
	// var_dump($_POST['id']);
	$id=divide($_POST['id']);
	$sql="SELECT `id_condsuic` ACCIONES,
...
$sql.="WHERE tipo_doc='".$id[1]."' AND documento='".$id[0];
	...
	return panel_content($datos["responseResult"],"condsuic-lis",5);
   }
...
function cmp_condsuic(){
  ...
  $c[]=new cmp('fecha_seg','d','10',$d,$w.' '.$o,'Fecha Seguimiento','fecha_seg',null,null,true,true,'','col-2','validDate(this,-32,0)');
  ...
  $c[]=new cmp('cons_micronutr','s','2',$d,$w.' '.$bl.' '.$ge.' '.$gp.' '.$o,'¿Consume Micronutrientes?','rta',null,null,false,$x,'','col-2');
    ...
    $c[]=new cmp('fecha_cierre','d','10',$d,$w.' cc '.$bl.' '.$no.' '.$o,'Fecha de Cierre','fecha_cierre',null,null,false,$x,'','col-15','validDate(this,-32,0);');
