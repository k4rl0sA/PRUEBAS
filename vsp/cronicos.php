...
function focus_cronicos(){
 ...
 FUNCTION seg_cronicos(){
	// var_dump($_POST['id']);
	$id=divide($_POST['id']);
	$sql="SELECT `id_cronicos` ACCIONES,
...
// var_dump($a);
// var_dump($rta);
	if ($a=='cronicos-lis' && $b=='acciones'){//a mnombre del modulo
		$rta="<nav class='menu right'>";	
    $rta.="<li class='icono editar' title='Editar' id='".$c['ACCIONES']."' Onclick=\"setTimeout(getData,500,'cronicos',event,this,['fecha_seg','numsegui','evento','estado_s','motivo_estado'],'cronicos.php');\"></li>";
	}
