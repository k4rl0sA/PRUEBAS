...
function focus_eraira(){
  return 'eraira';
 }
 
 
 function men_eraira(){
  $rta=cap_menus('eraira','pro');
  return $rta;
 }
 ...
 FUNCTION seg_eraira(){
	// var_dump($_POST['id']);
	$id=divide($_POST['id']);
	$sql="SELECT `id_eraira` ACCIONES,tipo_doc,documento,fecha_seg Fecha,numsegui Seguimiento,FN_CATALOGODESC(87,evento) EVENTO,FN_CATALOGODESC(73,estado_s) estado,cierre_caso Cierra,
...
$ob='Ob';
  ...
  $ev=$event[3];
	...
	$c[]=new cmp('id_eraira','h','50',$_POST['id'],$w.' '.$o,'Id de eraira','id_eraira',null,null,false,false,'','col-2');
  ...
  $c[]=new cmp('motivo_estado','s','3',$d,$w.' '.$o,'Motivo de Estado','motivo_estado',null,null,false,$x,'','col-2');  
    ...
    $c[]=new cmp('adecua_oxi','s','3',$d,$w.' '.$o,'¿Adecuada Administración de Inhalador/Oxigeno Domiciliario?','adecua_oxi',null,null,false,$x,'','col-2');
    $c[]=new cmp('adhe_tratam','s','3',$d,$w.' '.$o,'¿Adherencia al tratamiento?','adhe_tratam',null,null,false,$x,'','col-2');
    ...
    $c[]=new cmp('adhe_lavamano','s','2',$d,$w.' '.$o,'¿Se evidencia adherencia a técnica de lavado de manos?','rta',null,null,false,$x,'','col-3');
    $c[]=new cmp('reing_hospita','s','2',$d,$w.' '.$o,'Menor con reingreso hospitalario por este evento','rta',null,null,false,$x,'','col-3');
...
function opc_adhe_tratam($id=''){
return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=88 and estado='A' ORDER BY 1",$id);
}
function opc_adecua_oxi($id=''){
return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=88 and estado='A' ORDER BY 1",$id);
}
