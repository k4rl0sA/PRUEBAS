...
function focus_saludoral(){
 ...
 FUNCTION seg_saludoral(){
	// var_dump($_POST['id']);
	$id=divide($_POST['id']);
	$sql="SELECT `id_saludoral` ACCIONES,
...
$sql.="' ORDER BY fecha_create";
	...
	return panel_content($datos["responseResult"],"saludoral-lis",5);
   }
...
function gra_saludoral(){
  ...
  function get_saludoral(){
    if($_REQUEST['id']==''){
      ...
      fecha_seg,numsegui,evento,estado_s,motivo_estado,clasi_riesgo,sangra_cepilla,evide_anormal,explica_breve,urg_odonto,estrategia_1,estrategia_2,acciones_1,desc_accion1,acciones_2,desc_accion2,acciones_3,desc_accion3,activa_ruta,ruta,novedades,signos_covid,caso_afirmativo,otras_condiciones,observaciones,cierre_caso,fecha_cierre,mejora_practica,redu_riesgo_cierre
      FROM vsp_saludoral
...
// var_dump($a);
// var_dump($rta);
	if ($a=='saludoral-lis' && $b=='acciones'){//a mnombre del modulo
		$rta="<nav class='menu right'>";
    $rta.="<li class='icono editar' title='Editar' id='".$c['ACCIONES']."' Onclick=\"setTimeout(getData,500,'saludoral',event,this,['fecha_seg','numsegui','evento','estado_s','motivo_estado'],'saludoral.php');\"></li>";
	}
