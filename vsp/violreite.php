...
function focus_violreite(){
 ...
 FUNCTION seg_violreite(){
	// var_dump($_POST['id']);
	$id=divide($_POST['id']);
	$sql="SELECT `id_violreite` ACCIONES,
...
function gra_violreite(){
  ...
  function get_violreite(){
    if($_REQUEST['id']==''){
      ...
      $id=divide($_REQUEST['id']);
      $sql="SELECT concat(id_violreite,'_',tipo_doc,'_',documento,'_',numsegui,'_',evento),
...
// var_dump($a);
// var_dump($rta);
	if ($a=='violreite-lis' && $b=='acciones'){//a mnombre del modulo
