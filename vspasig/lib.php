...
function lis_vsp_asig(){
	...
	LEFT JOIN hog_geo D ON C.estrategia=D.estrategia AND C.sector_catastral=D.sector_catastral AND C.nummanzana=D.nummanzana AND C.predio_num=D.predio_num AND C.unidad_habit=D.unidad_habit AND C.estado_v=D.estado_v
	 WHERE C.estado_v in (1) AND C.subred in (SELECT C.subred FROM usuarios where id_usuario='".$_SESSION['us_sds']."') and (D.asignado='".$_SESSION['us_sds']."')".whe_vsp_asig();
	//  echo $sql1;
	...
	FN_CATALOGODESC(42,C.estrategia) estrategia,direccion,
	C.sector_catastral,
...
function cmp_vsp_asig(){
 ...
 $c[]=new cmp('estrato','t','3',$d['estrato'],$w.' '.$o,'Estrato','estrato',null,null,false,$u,'','col-2');
