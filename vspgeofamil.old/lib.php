...
function lis_fam_geo(){
	$info=datos_mysql("SELECT COUNT(*) total FROM vspgeo WHERE subred in(select subred from usuarios where id_usuario = '{$_SESSION['us_sds']}') ".whe_fam_geo());
	$total=$info['responseResult'][0]['total'];
	$regxPag=5;
	$pag=(isset($_POST['pag-fam_geo']))? ($_POST['pag-fam_geo']-1)* $regxPag:0;
  ...
  FROM  vspgeo WHERE subred in(select subred from usuarios where id_usuario = '{$_SESSION['us_sds']}')
  		 ";
	$sql.=whe_fam_geo();
...
function focus_fam_geo(){
 return 'fam_geo';
}


function men_fam_geo(){
 $rta=cap_menus('fam_geo','pro');
 return $rta;
}
...
function cap_menus($a,$b='cap',$con='con') {
  $rta = ""; 
  $acc=rol($a);
  if ($a=='fam_geo' && isset($acc['crear']) && $acc['crear']=='SI'){  
...
function opc_asignado($id=''){
	...
	return opc_sql("SELECT `id_usuario`,nombre FROM `usuarios` WHERE equipo IN('PROFAM') AND subred='".subred()."' ORDER BY 1",$id);
}
...
// var_dump($a);
// var_dump($rta);
	if ($a=='fam_geo' && $b=='acciones'){
		$rta="<nav class='menu right'>";		
		$rta.="<li class='icono mapa1' title='Editar Información Geografica' id='".$c['ACCIONES']."' Onclick=\"mostrar('fam_geo','pro',event,'','lib.php',7);\"></li>"; //setTimeout(hideExpres,1000,'estado_v',['7']);
	}
